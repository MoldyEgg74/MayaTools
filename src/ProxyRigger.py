...
class ProxyRigger:
    ...
    def CreateProxyRigFromSelectedMesh(self):
        ...
        if not skin:
            raise Exception(f"{mesh} has no skin! to tool only works with a rigged model")
        self.skin = skin[0]
    ...
    def CreateProxyModelForJntAndVerts(self, jnt, verts):
        ...
        labels = set() # a set is like a list, but only holds unique elements, and it is not ordered, faster than list when it comes to looking for stuff
        for face in faces:
...
class ProxyRiggerWidget(QMayaWindow):
    ...
    def GetWindowHash(self):
        return "8c4c1c2baf6ab766d772558c7bf5d7a6"
